...
func IdentifyProperties(instanceID string) gateway.IdentifyProperties {
	// Use instance ID in device identifier to make each installation unique
	deviceID := "discord-cli"
	if instanceID != "" {
		deviceID = fmt.Sprintf("discord-cli-%s", instanceID)
	}

	return gateway.IdentifyProperties{
		gateway.IdentifyDevice: deviceID,
...
func superProps(instanceID string) (string, error) {
	props := IdentifyProperties(instanceID)
